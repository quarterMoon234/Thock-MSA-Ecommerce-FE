...
백엔드는 MSA 기반 THOCK API와 연동되며, 상품 탐색, 장바구니, 주문, Toss Payments 결제, 지갑/입출금 로그, 판매자 상품 관리 흐름을 실제 쇼핑몰 사용 시나리오에 맞춰 구성했습니다.
...
메인 화면은 노출 상품과 검색 진입점을 중심으로 구성했습니다.
검색 화면에서는 카테고리 필터와 키워드 검색 결과를 함께 제공해 사용자가 원하는 키보드 상품을 빠르게 찾을 수 있도록 했습니다.
...
상품 등록 화면에서는 상품명, 카테고리, 가격, 할인가, 재고, 이미지, 상세 설명을 입력해 판매 상품을 생성합니다.
...
- 구매자와 판매자 역할을 모두 고려한 이커머스 화면을 구성했습니다.
- 상품 탐색부터 주문, 결제, 지갑 로그까지 실제 구매 흐름을 끝까지 연결했습니다.
- 백엔드의 주문, 결제, 재고, 취소 흐름을 프론트 화면에서 검증할 수 있도록 장바구니, 주문 내역, 주문 상세, 입출금 내역 화면을 구성했습니다.
- 판매자 센터를 통해 상품 등록, 상품 관리, 판매 수익 조회, 출금 신청을 지원해 단순 조회형 쇼핑몰이 아닌 운영 흐름까지 고려했습니다.
- Vite 프록시와 배포 환경 API 주소를 분리해 로컬 개발과 배포 환경을 모두 대응할 수 있도록 구성했습니다.
